(function () {
    ...
    function getToast() {
        if (!toastMixinPromise) {
            toastMixinPromise = loadSwal().then(function (Swal) {
                return Swal.mixin({
                    toast: true,
                    position: "top",
    ...
    $(document).ready(function () {
                ...
                success: function (response) {
                    ...
                    if (window.innerWidth >= 768 && response.total_cart_items !== undefined) {
                        window.dispatchEvent(
                            new CustomEvent("show-mini-cart", {
                                detail: {
        ...
        $(document).on(
            "change",
            ".search-filter, .category-filter, .rating-filter, input[name='price-filter'], input[name='prices'], input[name='items-display'], .size-filter, .colors-filter",
            function () {
        ...
        $(document).on("click", ".reset_shop_filter_btn", function () {
            ...
            if ($("#shop-filters-form").length && $("#shop-filters-form").attr("hx-get") && window.htmx) {
                ...
                htmx
                    .ajax("GET", url, { target: "#product-grid", swap: "innerHTML", headers: { "HX-Request": "true" } })
                    .then(function () {
